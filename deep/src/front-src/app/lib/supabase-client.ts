...
export async function getEstimates() {
  return await supabase.from("estimates").select(
    " id, created_at, title, description,user_id, description ",
  );
}
...
export async function getProjects() {
  const { data, error } = await supabase
    .from("projects").select(
      " id, created_at, title, description,user_id, description, emissions: calculation->emission, routes:  calculation->routes, chain: calculation",
